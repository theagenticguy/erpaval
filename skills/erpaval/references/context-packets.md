...
Every decision-bearing node writes a YAML packet to `.erpaval/sessions/<id>/`. Packets are read-only hints the orchestrator reads when composing Task-tool calls. The Task tools (`TaskCreate`, `TaskUpdate`, `TaskList`) hold authoritative state for "what's next, what's blocked, what's stuck." Per-task packets additionally double as research/ultraplan-style work logs — subagents edit them section-by-section using the `write-protocol.md` block.
...
- Frontmatter-style header (Status, AC source, agent name, isolation).
...
The plugin `PostToolUse(Write|Edit)` hook (`hooks/validate_packet.py`) runs on every write, early-exits on non-`.erpaval/` paths, and Pydantic-checks YAML packets plus per-task `.md` frontmatter against their schemas. On failure the hook injects a systemMessage via `additionalContext` — advisory, not blocking. Fail-open by construction so a hook bug can never wedge a session.
