...
- All role prompts carry the write-protocol block verbatim from `${CLAUDE_PLUGIN_ROOT}/skills/product-discovery/references/write-protocol.md`.
...
- `${CLAUDE_PLUGIN_ROOT}/skills/product-discovery/references/write-protocol.md` — pasted into every role prompt.
- `${CLAUDE_PLUGIN_ROOT}/skills/product-discovery/references/roles/` — nine role files (product-analyst, system-architect, research-scout, hmw-framer, ears-specifier, discovery-lead, jtbd-interviewer, prd-synthesizer, discovery-critic).
- `${CLAUDE_PLUGIN_ROOT}/skills/product-discovery/templates/` — seven skeletons (prd-template, worklog-skeleton, discovery-memo, jtbd-skeleton, double-diamond-workbook, hmw-skeleton, ears-spec-skeleton).
- `${CLAUDE_PLUGIN_ROOT}/skills/product-discovery/references/inference-heuristics.md` — for PRD-route signal parsing.
- `${CLAUDE_PLUGIN_ROOT}/skills/product-discovery/references/quality/prd.md` — PRD-route quality bar.
...
- `${CLAUDE_PLUGIN_ROOT}/skills/product-discovery/references/methodology/discovery-rounds.md` — six-phase methodology.

---

## Phase tracking

Before Phase 0, create 6 todo items — Route, Frame, Parallel execution, Synthesize, Critic review, Deliver. During Phase 2, additionally `TaskCreate` one item per role in the selected fan-out (e.g., product-analyst / system-architect / research-scout for the PRD route) and flip each to `completed` when its work log shows `Status: COMPLETE`. Flip phase items to `in_progress` on entry, `completed` on exit.
...
Also gate: if the brief is genuinely ambiguous (audience unknown, source material unclear), use one `AskUserQuestion` call with up to 4 questions to resolve. Then proceed. Do not loop.
...
1. Parse the user's idea using `${CLAUDE_PLUGIN_ROOT}/skills/product-discovery/references/inference-heuristics.md`. Extract every signal (app type, core features, personas, scale, deployment, auth, data persistence, complexity).
2. Build the Intent Profile table. Present it inline with confidence levels.
3. Ask up to 3-4 genuine-ambiguity questions via `AskUserQuestion`. Do not ask about tech choices (that's `/build-stack`) or obvious things the user already said.
...
<role_reference>
${CLAUDE_PLUGIN_ROOT}/skills/product-discovery/references/roles/discovery-lead.md
</role_reference>
...
Launch three agents in one message. Each writes to its own work log; all three contribute sections to the PRD that the synthesizer assembles in Phase 3.
...
<role_reference>
${CLAUDE_PLUGIN_ROOT}/skills/product-discovery/references/roles/{{ role }}.md
</role_reference>
...
<reference_material>
${CLAUDE_PLUGIN_ROOT}/skills/product-discovery/references/inference-heuristics.md
${CLAUDE_PLUGIN_ROOT}/skills/product-discovery/references/quality/prd.md
${CLAUDE_PLUGIN_ROOT}/skills/product-discovery/templates/prd-template.md
</reference_material>
...
Launch whatever framing.md flagged in one message. Write role prompts from the corresponding role files.

### HMW-only route — one agent

Spawn `hmw-framer` as a single foreground `Agent`:
...
<role_reference>
${CLAUDE_PLUGIN_ROOT}/skills/product-discovery/references/roles/hmw-framer.md
</role_reference>
...
<reference_material>
${CLAUDE_PLUGIN_ROOT}/skills/product-discovery/references/frameworks/how-might-we.md
</reference_material>
...
Spawn `ears-specifier` as a single foreground `Agent`:
...
<role_reference>
${CLAUDE_PLUGIN_ROOT}/skills/product-discovery/references/roles/ears-specifier.md
</role_reference>
...
<reference_material>
${CLAUDE_PLUGIN_ROOT}/skills/product-discovery/references/frameworks/ears.md
</reference_material>
...
Spawn `jtbd-interviewer` as a single foreground `Agent`:
...
<role_reference>
${CLAUDE_PLUGIN_ROOT}/skills/product-discovery/references/roles/jtbd-interviewer.md
</role_reference>
...
<reference_material>
${CLAUDE_PLUGIN_ROOT}/skills/product-discovery/references/frameworks/jtbd-job-stories.md
</reference_material>
...
A work log with unchanged line count across two consecutive check-ins is stuck. Launch a fresh `Agent` with the existing file state and a "skip completed sections" prompt so the new agent doesn't re-do work. Never wait for a stuck agent to self-correct.
...
Spawn `prd-synthesizer` as a foreground `Agent`:

```text
You are the PRD synthesizer. Read the three parallel work logs and compose the final PRD.

<role_reference>
${CLAUDE_PLUGIN_ROOT}/skills/product-discovery/references/roles/prd-synthesizer.md
</role_reference>
...
The discovery-lead runs a second pass, this time composing `discovery-memo.md` from the Phase 2 outputs it orchestrated. Seed from `templates/discovery-memo.md`.
...
Spawn `discovery-critic` as a foreground `Agent`:

```text
You are the discovery critic. Read the synthesized artifact and write a rubric-graded review.

<role_reference>
${CLAUDE_PLUGIN_ROOT}/skills/product-discovery/references/roles/discovery-critic.md
</role_reference>
