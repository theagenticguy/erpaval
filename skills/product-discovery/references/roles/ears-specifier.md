...
Erpaval's Plan phase walks `spec.md` and emits Task-tool calls:

```text
for each user_story in spec:
  for each ac in user_story.acs:
    task_id = TaskCreate(
      subject = "T-{ac.id}: {short summary}",
      description = "Implement {ac.id}:\n{ac.text}"
    )

    if ac has Dependencies:
      blocker_ids = [lookup_task_id(dep) for dep in ac.dependencies]
      TaskUpdate(taskId=task_id, addBlockedBy=blocker_ids)

    if ac has [P]:
      record agent_isolation = "worktree" in CP-TASK-N
```
...
- See `${CLAUDE_PLUGIN_ROOT}/skills/product-discovery/references/frameworks/ears.md` for the full framework reference including worked examples.
