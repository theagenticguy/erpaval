...
# ///
"""agentSpawn hook: emit category counts + INDEX pointer when this repo
has recent ERPAVal activity.

Kiro `agentSpawn` captures STDOUT into the agent's starting context, so we
print the summary and exit 0. Identical firing gates to the Claude Code
SessionStart port.
...
Firing gates (ALL must hold):
  ...
  3. Fires at most once per Kiro session (HookState `bootstrapped`).

Skip conditions:
  - No .erpaval/ at all -> silent no-op.
...
def handle(input, state: HookState) -> None:
    if not isinstance(input, AgentSpawnInput):
        return
