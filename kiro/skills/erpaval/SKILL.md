...
| `kiro_validate_packet.py`         | `postToolUse` (matcher=`write`) | **Advisory** Pydantic schema-check on writes under `.erpaval/` — `postToolUse` cannot block, so a malformed packet is warned, not rejected. Early-exits on non-erpaval paths. Also drops `/tmp/kiro-erpaval-active-<session_id>` as a cross-hook "this Kiro session touched ERPAVal" marker.                         |
...
| Hook event names     | `SessionStart`, `PostToolUse(Write\|Edit\|MultiEdit)`, `Stop`                   | `agentSpawn`, `postToolUse(matcher=write)`, `stop`                                                                                                                                                                                                                                                                         |
