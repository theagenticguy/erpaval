...
description: >
  ...
  product-strategy.
license: MIT
compatibility: Designed for Kiro CLI (open Agent Skills standard). Skills are auto-exposed as slash commands; arguments are accessed via Kiro's `$ARGUMENTS` (full string) and `${N}` (Nth token) substitution.
---

> **Slash-command argument convention.** Kiro auto-exposes every skill as a slash command. When invoked as `/product-discovery <source>`, the body's `$ARGUMENTS` (or `${1}`) substitution resolves to the source path or topic — typically a path to raw content (brief, notes, tickets) or a one-line topic. Optional; with no arguments the orchestrator prompts the user.
...
| `assets/hmw-skeleton.md`                     | HMW brainstorms file — `brainstorms/NNN-<slug>-requirements.md`                                          |
| `assets/jtbd-skeleton.md`                    | JTBD job-stories file                                                                                    |
| `assets/ears-spec-skeleton.md`               | EARS spec file — `specs/NNN-<slug>/spec.md`                                                              |

# Product discovery

One skill, five routes. Takes a problem at any maturity (one-sentence idea, research notes, fuzzy framing, clear feature) and routes to the right framework pipeline. File-first write protocol throughout. Every role is a Markdown prompt copied verbatim into a general-purpose Kiro `/spawn` invocation — zero custom agent files needed.
...
When the brief is genuinely ambiguous (audience unknown, source unclear), ask the user up to 4 clarifying questions in a single message. Then proceed. Do not loop.
...
- Discovery: `${ERPAVAL_HOME}/skills/product-design-shared/references/double-diamond.md`, `frameworks/how-might-we.md`, `frameworks/jtbd-job-stories.md`.
