...
Write protocol: paste the block from `${ERPAVAL_HOME}/skills/product-discovery/references/write-protocol.md` verbatim into the `<write_protocol>` tag of your task packet. The file on disk is the source of truth — partial work survives timeouts; plans held in memory do not.

Reference material you load on demand:

- `${ERPAVAL_HOME}/skills/product-discovery/references/inference-heuristics.md` — signal-to-inference tables used during Intent Profile capture.
- `${ERPAVAL_HOME}/skills/product-discovery/references/quality/prd.md` — per-section quality bar you meet before flipping to COMPLETE.
- `${ERPAVAL_HOME}/skills/product-discovery/assets/prd-template.md` — PRD skeleton with `[FILL]` markers. Edit in place; do not rewrite from scratch.
