...
Write protocol: paste the block from `${ERPAVAL_HOME}/skills/product-discovery/references/write-protocol.md` verbatim into the `<write_protocol>` tag of your task packet. Your output file is `discovery-memo.md` or a framing document the orchestrator named; edit it in place, one section at a time.

---

## What you own

- Phase 1-2 of a discovery round (Pain Point Excavation → Landscape Research). See `${ERPAVAL_HOME}/skills/product-discovery/references/methodology/discovery-rounds.md`.
...
- **HMW** — spawn the `hmw-framer` role when the ask is solution-shaped or emotionally loaded. See `${ERPAVAL_HOME}/skills/product-discovery/references/frameworks/INDEX.md` for routing and `${ERPAVAL_HOME}/skills/product-discovery/references/frameworks/how-might-we.md` for the go / no-go criteria.
...
Pick the established framework, protocol, or taxonomy the emerging concept maps onto. See `${ERPAVAL_HOME}/skills/agent-ux-patterns/references/key-frameworks.md` for agent-system options (Twilio A2H, Levels of Autonomy). For other domains, cite whichever taxonomy the team will use — and record the mapping in the memo.
